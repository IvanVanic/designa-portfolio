...
export function AnimatedSection({ children, className = "", animation = "fadeUp", delay = 0 }: AnimatedSectionProps) {
  ...
  const getAnimationClasses = () => {
    ...
    switch (animation) {
      case "fadeUp":
        return `${baseClasses} ${isIntersecting ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`
      case "fadeIn":
        return `${baseClasses} ${isIntersecting ? "opacity-100" : "opacity-0"}`
      case "slideLeft":
        return `${baseClasses} ${isIntersecting ? "opacity-100 translate-x-0" : "opacity-0 translate-x-8"}`
      case "slideRight":
        return `${baseClasses} ${isIntersecting ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-8"}`
      default:
        return baseClasses
